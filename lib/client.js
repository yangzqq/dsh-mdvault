...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			'.mdv-plain{margin:0;padding:12px 14px;background:rgba(127,127,127,.08);border:1px solid rgba(127,127,127,.22);border-radius:8px;overflow:auto;max-height:calc(100vh - 260px);font-family:ui-monospace,Consolas,monospace;font-size:12.5px;line-height:1.6;white-space:pre;tab-size:2}',
		...
		function resolveOpenTarget(rawTarget, currentPath, kind, fileList) {
			...
			if (!rel) return null;
			if (SHEET_RE.test(name)) return { kind: 'sheet', rel: rel, frag: frag };
			if (PDF_RE.test(name)) return { kind: 'pdf', rel: rel, frag: frag };
			if (IMAGE_RE.test(name)) return { kind: 'image', rel: rel, frag: frag };
			if (HTML_RE.test(name)) return { kind: 'html', rel: rel, frag: frag };
			if (TEXT_RE.test(name)) return { kind: 'text', rel: rel, frag: frag };
			return { kind: 'file', rel: rel, frag: frag };
		}

		/** The markdown-family extensions, treated as interchangeable when matching. */
		const MD_FAMILY_RE = /\.(md|markdown|mdx)$/i;
		...
		function findByPathIn(list, p) {
			...
			if (MD_FAMILY_RE.test(pl)) {
				const stem = pl.replace(MD_FAMILY_RE, '');
				for (const f of list) {
					const fp = f.path.toLowerCase();
					if (MD_FAMILY_RE.test(fp) && fp.replace(MD_FAMILY_RE, '') === stem) return f;
				}
			}
			return null;
		}
		...
		function MarkdownView(props) {
			...
			React.useEffect(function () {
				...
				const onClick = function (event) {
					...
					// The path segment records the dialect: /w/ wikilink,
					// /r/ document-relative markdown link.
					let kind;
					let encoded;
					if (href.indexOf(WIKI_PREFIX) === 0) { kind = 'wiki'; encoded = href.slice(WIKI_PREFIX.length); }
					else if (href.indexOf(REL_PREFIX) === 0) { kind = 'rel'; encoded = href.slice(REL_PREFIX.length); }
					else return;
					event.preventDefault();
					event.stopPropagation();
					navRef.current.openFile(decodeTarget(encoded), kind);
				};
		 ...
		 * (so it has no writable selection). Fetches through the JSON API and
		 * renders with CodeView, keeping the same size tiers.
		 */
		function TreeTextView(props) {
			...
			React.useEffect(function () {
				let alive = true;
				setLoad({ status: 'loading' });
				api('read', { sessionId: props.nav.sessionId, path: props.rel }).then(function (res) {
					if (!alive) return;
			...
			}, [props.rel, props.nav.sessionId]);

			if (load.status === 'loading') return React.createElement('div', { className: 'mdv-empty' }, '正在读取 ' + props.rel + ' …');
			if (load.status === 'error') return React.createElement('div', { className: 'mdv-empty' }, '\u26A0\uFE0F ' + load.message);
		...
		function MdVaultView(props) {
			...
			function openFileImpl(rawTarget, kind) {
				...
				// Notes open as the editable document; the tree must contain
				// them, so fall back to the raw asset when it does not.
				if (target.kind === 'note') {
					const hit = findByPath(target.rel);
					if (hit) { setAsset(null); setEditing(false); setSelected(hit.path); return true; }
					setAsset({ kind: 'download', name: target.rel.split('/').pop(), url: assetUrlFor(sessionId, target.rel, '') });
					return true;
				}
				...
				if (target.kind === 'text') {
					// Text opens in the editor when the tree lists it (which also
					// gives it a writable path); otherwise show it read-only.
					const hit = findByPath(target.rel);
					if (hit) { setAsset(null); setEditing(false); setSelected(hit.path); return true; }
					setAsset({ kind: 'text', name: name, rel: target.rel, url: assetUrlFor(sessionId, target.rel, '') });
					return true;
				}
				if (target.kind === 'pdf') {
					setAsset({ kind: 'pdf', name: name, url: assetUrlFor(sessionId, target.rel, target.frag), frag: target.frag });
					return true;
				}
				if (target.kind === 'image') {
					setAsset({ kind: 'image', name: name, url: assetUrlFor(sessionId, target.rel, '') });
					return true;
				}
				if (target.kind === 'html') {
					setAsset({ kind: 'html', name: name, url: assetUrlFor(sessionId, target.rel, '') });
					return true;
				}
				setAsset({ kind: 'download', name: name, url: assetUrlFor(sessionId, target.rel, '') });
				return true;
			}
			...
			function renderDir(node, depth) {
				...
				for (const dn of dirNames) {
					...
					// A filter forces every matching branch open; otherwise only
					// the path down to the current document is expanded.
					const isOpen = filter.trim() ? true : isDirOpen(child.path, collapsed, selected);
			...
			// Building the tree creates one React element per file and directory.
			// Memoized: otherwise every keystroke in the filter box, and every
			// document switch, rebuilt every element for the whole workspace.
			// `visible`/`collapsed`/`selected`/`openFromTree` are the only inputs
			// that can change what a row looks like.
			const treeRows = React.useMemo(function () {
				if (listErr || files === null || !fileList.length || !visible.length) return null;
				return renderDir(buildTree(), 0);
				// eslint-disable-next-line react-hooks/exhaustive-deps
			}, [listErr, files, fileList, visible, collapsed, selected, filter]);
