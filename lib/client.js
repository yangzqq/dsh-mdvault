...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** True for a destination carrying a URL scheme (`https:`, `data:`, `blob:`). */
		function hasUrlScheme(dest) {
			return /^[a-z][a-z0-9+.-]*:/i.test(dest)
		}

		/**
		 * Resolve a markdown image destination to a workspace-relative path,
		 * or null when it is not a local file.
		 *
		 * Remote schemes are refused rather than coerced: without this guard a
		 * destination such as `https://host/a.png` is chopped into the bogus
		 * relative path `https:/host/a.png`. The platform currently short-
		 * circuits absolute URLs before consulting us, so this cannot be reached
		 * through `MarkdownText` today — but it keeps the function correct on
		 * its own terms instead of depending on the caller's ordering.
		 */
		function toRelPath(dest, currentPath) {
			const raw = String(dest == null ? '' : dest).trim();
			if (!raw || hasUrlScheme(raw)) return null;
			return resolveRelPath(raw, dirNameOf(currentPath));
		}
