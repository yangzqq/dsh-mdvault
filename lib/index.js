...
const XLSX_DIST = join(dirname(fileURLToPath(import.meta.url)), '..', 'dist', 'xlsx.full.min.js')
...
const TREE_RE = /\.(md|markdown|pdf|xlsx|xlsm|xls|csv|png|jpe?g|gif|svg|webp)$/i
...
function contentTypeOf(path) {
	...
	if (/\.(txt|log)$/.test(p)) return 'text/plain; charset=utf-8'
...
export function apply(ctx) {
		...
		handler: async (req, res) => {
			try {
				...
				if (method === 'read') {
					...
					sendJson(res, 200, { ok: true, path: rel, text })
					return
				}
				if (method === 'write') {
					const rel = safeRel(body.path)
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (_req, res) => {
			try {
				const lib = await readFile(XLSX_DIST)
				res.writeHead(200, {
					'Content-Type': 'application/javascript; charset=utf-8',
					'Content-Length': lib.byteLength,
					'Cache-Control': 'no-store',
				})
				res.end(lib)
			} catch (err) {
